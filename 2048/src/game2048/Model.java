...
public class Model {
    ...
    public boolean emptySpaceExists() {
        ...
        for (int x=0; x < board.size(); x++){
            for (int y=0; y < board.size(); y++){
                if (board.tile(x, y) == null || board.tile(x,y).value() == 0){
                    return true;
                }
            }
        }
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int x=0; x < board.size(); x++){
            for (int y=0; y < board.size(); y++){
                int currentValue = board.tile(x, y).value();
                // Check left
                if (x > 0 && currentValue == board.tile(x - 1, y).value()) {
                    return true;
                }
                // Check right
                if (x < board.size() - 1 && currentValue == board.tile(x + 1, y).value()) {
                    return true;
                }
                // Check up
                if (y > 0 && currentValue == board.tile(x, y - 1).value()) {
                    return true;
                }
                // Check down
                if (y < board.size() - 1 && currentValue == board.tile(x, y + 1).value()) {
                    return true;
                }
            }
        }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        int targetY = y;
        for (int cur = y + 1 ; cur < board.size(); cur ++){
            if( board.tile(x, cur) == null || board.tile(x, cur).value() == 0){
                targetY = cur;
            }else if (board.tile(x, cur).value() == myValue && !board.tile(x, cur).wasMerged()){
                targetY = cur;
                score += myValue * 2;
            }
        }
    ...
    public void tiltColumn(int x) {
        ...
        for (int curY= board.size()-2 ; curY >= 0; curY--){
            if (board.tile(x, curY) != null && board.tile(x, curY).value() != 0){
                this.moveTileUpAsFarAsPossible(x, curY);
            }
        }
    }

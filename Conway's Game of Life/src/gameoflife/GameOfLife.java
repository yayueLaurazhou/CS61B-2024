...
public class GameOfLife {
    ...
    public TETile[][] nextGeneration(TETile[][] tiles) {
        ...
        // The new board is filled with Tileset.NOTHING
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                ...
                for (int k = 0; k < 8; k++) {
                    ...
                    if (isInBounds(newRow, newCol, width, height)) {
                        if (tiles[newRow][newCol].equals(Tileset.CELL)){
                            numNeighbors += 1;
                        }
                    }
                }
    ...
    public void saveBoard() {
        ...
        for (int i = height - 1; i >= 0; i--) {
            for (int j = 0; j < width; j++) {
                if (currentState[j][i].equals(Tileset.CELL)) {
                    matrix.append("1");
                } else {
                    matrix.append("0");
                }
        ...
        writeFile("src/save.txt", matrix.toString());
    }
    ...
    public TETile[][] loadBoard(String filename) {
        ...
        int width = Integer.parseInt(splitResult[0].split(" ")[0]);
        int height =  Integer.parseInt(splitResult[0].split(" ")[1]);
        TETile[][] loadResult = new TETile[width][height];
        for (int i = 0; i < height; i ++){
            for (int j = 0; j < width; j ++){
                if (splitResult[i+1].charAt(j) == 1){
                    loadResult[i][j] = Tileset.CELL;
                } else {
                  loadResult[i][j] = Tileset.NOTHING;
                }
            }
        }
        return loadResult;
